...
async function main() {
	...
	console.log("Deploying WETH stake token");
	...
	await weth.deployed();
	console.log("WETH stake token deployed at:", weth.address);
	...
	writeFileSync(
		...
		`${JSON.stringify({ rollupCreator: contracts.rollupCreator.address, stakeToken: weth.address }, null, 2)}\n`,
	);
}
